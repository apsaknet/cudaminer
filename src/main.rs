...
use crate::cli::Opt;
use crate::client::grpc::ApaskdHandler;
...
const WHITELIST: [&str; 4] = ["libapsakcuda", "libapsakopencl", "apsakcuda", "apsakopencl"];
...
async fn get_client(
    apsakd_address: String,
    mining_address: String,
    mine_when_not_synced: bool,
    block_template_ctr: Arc<AtomicU16>,
) -> Result<Box<dyn Client + 'static>, Error> {
    if apsakd_address.starts_with("stratum+tcp://") {
        let (_schema, address) = apsakd_address.split_once("://").unwrap();
        ...
        .await?)
    } else if apsakd_address.starts_with("grpc://") {
        Ok(ApaskdHandler::connect(
            apsakd_address.clone(),
            mining_address.clone(),
            mine_when_not_synced,
            Some(block_template_ctr.clone()),
        )
...
async fn main() -> Result<(), Error> {
    ...
    let (app, mut plugin_manager): (App, PluginManager) = apsak_miner::load_plugins(Opt::into_app(), &plugins)?;
    ...
    info!("=================================================================================");
    info!("                 apsaK-Miner GPU {}", env!("CARGO_PKG_VERSION"));
